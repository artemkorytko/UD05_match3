...
namespace Match3
{
    public class BoardController : IDisposable
    {
        ...
        private async void OnElementClick(OnElementClickSignal signal)
        {
            ...
            else
            {
                if (IsCanSwap(_firstSelected, element))
                {
                    ...
                    _firstSelected = null;
                     await CheckBoard(); // он отработал и вы про его забыли се... Forget()
                }
                else
                {
        ...
        private async UniTask CheckBoard()  // таска без позврата  - там где мы вызываем этот метод нужно добавить .Forget()
        {
            _isBlocked = true;
            ...
            do
            {
                ...
                if (elementsForCollecting.Count > 0)
                {
                    ...
                    _signalBus.Fire(new OnBoardMatchSignal(elementsForCollecting.Count)); // передаем сигнал скок мы собрали элементов
        ...
        private async UniTask NormalizeBoard()
        {
            ...
            List<UniTask> tasks = new List<UniTask>(); // тож самое что и в выключении элементов (ниже смтри)
            for (int y = row - 1; y >= 0; y--)
            {
                for (int x = column - 1; x >= 0; x--)
                {
        ...
        {
            List<UniTask> tasks = new List<UniTask>(); // эт сделано для того чтоб запустить все таски (в List<Element> elementsForCollecting) одновременно
            foreach (var element in elementsForCollecting)
                tasks.Add( element.Disable()); 

            await UniTask.WhenAll(tasks); // тут мы запускаем наши таски иииииии ждемс кода все отработают
        }
        ...
        private async UniTask Swap(Element first, Element second)
        {
            ...
            tasks.Add(second.SetLocalPosition(first.transform.localPosition, first.GridPosition));
            tasks.Add(first.SetLocalPosition(position, gridPosition));
            await UniTask.WhenAll(tasks);
        }
